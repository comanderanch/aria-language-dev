# ARIA DIAGNOSTIC — CONTROLLED UNMASK PHASE 3
# ============================================================
# PURPOSE:
#   Expand ghost token access to GREEN boundary.
#   Add 75 GREEN-adjacent ghost tokens to existing
#   VIOLET + BLUE unmask (150 total so far).
#   Total unmasked ghosts: ~225 maximum.
#
# PHASE RECAP:
...
#
# WHY GREEN:
#   GREEN plane = growth, vitality, forward motion
#   Different spectrum from VIOLET/BLUE — adds energy range
#   Covers narrative momentum vocabulary
#   Freq 0.65 — mid-range — strong but not urgency tier
#
# RULES:
#   Do NOT remove mask entirely
#   Do NOT exceed ~250 total unmasked ghosts
...
#
# April 7 2026 — aria-language-dev
...
print("\nARIA DIAGNOSTIC — CONTROLLED UNMASK PHASE 3")
print("GREEN boundary — adding 75 ghost tokens")
print("=" * 60)
...
checkpoint_path = Path(__file__).parent / "training/checkpoints/round132_best.pt"
...
# ---------- BUILD GREEN REFERENCE VECTOR ----------
print("\nBuilding GREEN plane reference vector...")

green_vecs = []
for word, wid in tokenizer.vocab.items():
    if wid >= vocab_size:
        continue
    if tokenizer.word_to_plane.get(word) == "GREEN":
        vec  = embed_weights[wid]
        norm = torch.norm(vec).item()
        if norm >= 0.5:
            green_vecs.append(vec)

if not green_vecs:
    print("ERROR: No GREEN anchors found.")
    sys.exit(1)

green_ref = torch.stack(green_vecs).mean(dim=0)
green_ref = green_ref / (torch.norm(green_ref) + 1e-8)
print(f"GREEN reference built from {len(green_vecs)} anchored words")

# ---------- SCORE REMAINING GHOSTS BY GREEN SIMILARITY ----------
print("Scoring remaining ghost slots by GREEN similarity...")
...
# ---------- PATCH INDEX ----------
print("\nPatching aria_token_index.json with GREEN ghost markers...")
...
print(f"Patched {patched_count} GREEN ghost tokens into index")
...
print()
print("=" * 60)
print("PHASE 3 UNMASK SUMMARY")
print("=" * 60)
print(f"""
  Phase 1 (VIOLET):     75  tokens — identity/love anchor
  Phase 2 (BLUE):       75  tokens — depth/structure
  Phase 3 (GREEN):      {len(candidates)}  tokens — growth/narrative
  Total ghost access:   ~{len(prior_tids) + len(candidates)}  tokens

  WHY GREEN:
  GREEN plane = growth, vitality, forward narrative motion
  Freq 0.65 — mid-range energy tier
  Expands output into action/momentum vocabulary
  Different spectrum from VIOLET+BLUE — adds range
  ...
  Resume training with:
    - Expanded vocabulary (2,098 tokens, UNK 11.12%)
    - 3 unmask phases active (225 ghost tokens)
    - round132_best.pt as anchor (loss 3.553210)

  python3 run_safe_training.sh
""")

print("Phase 3 unmask complete.")
print("VIOLET + BLUE + GREEN boundary open.")
print()
