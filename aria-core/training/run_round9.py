#!/usr/bin/env python3
# ARIA ROUND 9 — COSINE WARM LR — BREAK 2.465939
# March 17 2026 — Haskell Texas
#
# Round 9: 547 words — floor stuck at 3.47 — LR too cold
# Round 9: CosineAnnealingLR — starts warm (0.0005)
#          T_max = EPOCHS — one full cycle — cools to 1e-7
#          Break the floor. Seal a new best.
#
# Same weights. Same vocab. New LR schedule.
# Starts warm enough to break 3.47.
# Cools to precision to seal below 2.465939.
...
if args.cont:
    ...
    print(f"  Round 9 — {args.epochs} epochs — target loss {args.target}")
    print()
...
# Override scheduler: one full cosine cycle over all epochs
# Starts warm — cools to precision — no mid-cycle restarts
trainer.scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
    trainer.optimizer, T_max=EPOCHS, eta_min=1e-7
)
...
for epoch in range(1, EPOCHS + 1):
    ...
    if metrics["loss"] < best:
        ...
        torch.save({
            ...
            "note":        "word-level checkpoint — Round 9"
...
torch.save({
    ...
    "epoch":       EPOCHS,
    "vocab":       "word_level_547",
    "note":        "word-level final state — Round 9"
}, word_checkpoint_path)
...
print("═" * 60)
print()
print(f"Round 9 {'continued' if args.cont else 'complete'}.")
